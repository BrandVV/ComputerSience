...
class UserDoor extends StatelessWidget {
                  ...
                  child: Column(
                    ...
                    children: [
                      ...
                      createButton(context, "Öffnen", "open_door",
                          returnDarkmodeButtonColor(darkmode)),
            ...
            onTap: () async {
              ...
              if (ziel == "home") {
                Navigator.push(
                    context,
                    MaterialPageRoute(
                        builder: (context) => UserHome(darkmode: darkmode)));
                ...
                if (doorController.text == null) {
                  ...
                  getAlert(context, "Fehler", "Du musst eine Zeit angeben");
                    ...
                    doorController.text != null) {
                  var response = await openDoor(context, doorController.text);
                  if (response == 404) {
                    response ??= "Unbekannt";
                    getAlert(context, "Fehler: $response",
                        "Die API hat den Fehlercode: *$response* zurückgegeben. Bitte kontaktieren Sie den Entwickler");
                  }
                }
              }
            },
  ...
  openDoor(context, String zeit) async {
    ...
    if (zeit != "" && zeit_int != 0) {
      if (zeit_int > 5) {
        getAlert(context, "Fehler",
            "Die Zeit darf nicht länger als 5 Sekunden sein");
      } else {
        if (zeit_int < 0) {
          ...
          try {
            ...
            if (response.statusCode == 200 || response.statusCode == 201) {
              getAlert(context, "Erfolg: ${response.statusCode}",
                  "Die Tür ist gleich offen");
              return response.statusCode;
            } else {
              print(response.statusCode);
              getAlert(context, "Fehler: ${response.statusCode}",
                  "Die API hat den Fehlercode: ${response.statusCode} zurückgegeben. Bitte kontaktieren Sie Niklas");
            }
          } on Exception {
            return 404;
          }
